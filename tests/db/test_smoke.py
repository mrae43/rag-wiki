...
def test_base_imports() -> None:
    """Verify Base, TimestampMixin, and UUIDMixin import without error."""
...
def test_base_is_declarative() -> None:
    """Verify Base is a subclass of SQLAlchemy's DeclarativeBase."""
    assert issubclass(Base, DeclarativeBase)


async def test_db_connection(engine: AsyncEngine) -> None:
    """Verify the database engine can execute a trivial query."""
...
async def test_pgvector_extension(engine: AsyncEngine) -> None:
    """Verify the pgvector extension is installed on the database."""
    async with engine.connect() as conn:
