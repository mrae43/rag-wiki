...
class GoogleAIProvider(EmbeddingProvider):
    ...
    async def embed(
        ...
        # NOTE: embedContentConfig is intentionally NOT sent here. The
        # batchEmbedContents endpoint does not accept it — it's only valid
        # on the single embedContent endpoint. Both outputDimensionality and
        # taskType are model-side defaults; the model returns 3072-dim vectors
        # matching the pgvector column without any config.
